/*
 * Versão 2: CAVEIRA
 * - WPM baixo  → Caveira piscando devagar
 * - WPM médio  → Caveira piscando normal
 * - WPM alto   → Caveira piscando rápido (modo doom!)
 */
...
// ── Sprites caveira ────────────────────────────
// Caveira com olhos abertos
static const char PROGMEM skull_open[] = {
    ,
};

// Caveira com olhos fechados (piscando)
static const char PROGMEM skull_closed[] = {
    ,
};
...
// ── OLED Direito: Caveira piscando ─────────────
void oled_render_right(void) {
    uint8_t wpm = get_current_wpm();

    // Velocidade de piscar baseada no WPM
    // WPM baixo = pisca devagar, WPM alto = pisca rapido
    uint32_t blink_interval;
    if (wpm < 30) {
        blink_interval = 1000; // 1 segundo
    } else if (wpm < 60) {
        blink_interval = 500;  // 0.5 segundo
    } else if (wpm < 90) {
        blink_interval = 250;  // 0.25 segundo
    } else {
        blink_interval = 100;  // 0.1 segundo - DOOM MODE!
    }

    bool eyes_open = (timer_read32() % (blink_interval * 2)) < blink_interval;

    if (eyes_open) {
        oled_write_raw_P(skull_open, sizeof(skull_open));
    } else {
        oled_write_raw_P(skull_closed, sizeof(skull_closed));
    }
}
